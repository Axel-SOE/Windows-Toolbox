...
function Invoke-BasicConfigMenu {
    ...
    do {
        $ChoiceMenu = Show-Menu -Title "Basic Config" -MenuOptions @(
            ...
            "",
            "7: Full Windows update (excluding drivers)",
            "8: Interactive console"
        )
        switch ($ChoiceMenu) {
            '1' { Set-ComputerName }
            '2' {
                $PCName = Read-Host -Prompt "`n   New ComputerName"        
...
function Start-WindowsUpdate {
    ...
    .DESCRIPTION
        Installeert alle beschikbare Windows Updates via PSWindowsUpdate (exclusief drivers).
    ...
    try {
        ...
        Write-Host "`n    $($updates.Count) update(s) found:" -ForegroundColor Green
        $updates | ForEach-Object {
            Write-Host "    - $($_.Title)" -ForegroundColor Cyan
        }
        
        $Confirmation = (Read-Host -Prompt "`n   Reboot after update(s) if required? y/n").ToLower()
        if ($Confirmation -eq 'y') {
            Enter-AutoLogin
            Enter-RunOnce
            Write-Host "`n    Installing updates and possible rebooting..." -ForegroundColor Yellow
            $updates | Get-WindowsUpdate -Install -AcceptAll -AutoReboot
        }
        else {
            Write-Host "`n    Installing updates" -ForegroundColor Yellow
            $updates | Get-WindowsUpdate -Install -AcceptAll
        }
