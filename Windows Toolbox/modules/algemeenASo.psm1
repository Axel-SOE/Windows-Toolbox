...
function Start-WindowsUpdate {
    ...
    try {
        ...
        if (-not $updates) {
            Write-Host "    [i] No updates available, skipping reboot." -ForegroundColor Yellow
            Start-Sleep -Seconds 2
            return
        }
        ...
        $Confirmation = (Read-Host -Prompt "`n   Reboot after update(s)? y/n").ToLower()
        if ($Confirmation -eq 'y') {
            Enter-AutoLogin
            Enter-RunOnce
            Write-Host "`n    Installing updates and rebooting..." -ForegroundColor Yellow
